...
class FirmwareHandler(Resource):
	...
	def __init__(self, firmware_dir):
		...
		for fname in self.firmware_files:
			...
			fetchFile("https://raw.githubusercontent.com/digistump/OakSoftAP/master/config.html", "./data/static/index.html")
			f = open("./data/static/index.html",'r')
			filedata = f.read()
			f.close()

			newdata = filedata.replace("var updateServerMode = 1;","var updateServerMode = 3;")

			f = open("./data/static/index.html",'w')
			f.write(newdata)
			f.close()
